...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def _regression_or_discrete_prediction(
        ...
        if action_head is not None:
            if mvae_decoder is not None and hasattr(action_head, 'predict_latent'):
                # u_t head: predict latent → decode via MVAE decoder
                u_pred = action_head.predict_latent(
                    multi_layer_hidden_states,
                    proprio=proprio,
                    proprio_projector=proprio_projector,
                    phase="Inference",
                )  # (1, latent_dim)
                decoded = mvae_decoder(u_pred.float())  # (1, 8, 7)
                # Gripper fix: MVAE outputs [-1,1], pipeline expects [0,1]
                decoded[:, :, 6] = (decoded[:, :, 6] + 1) / 2
                normalized_actions = decoded.squeeze(0).cpu().detach().numpy()  # (8, 7)
            else:
                # Standard L1 regression prediction
