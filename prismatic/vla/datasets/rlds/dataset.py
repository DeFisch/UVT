...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
        ...
        # pass through villax_latent if present in observation
        if "villax_latent" in old_obs:
            new_obs["villax_latent"] = tf.cast(old_obs["villax_latent"], tf.float32)
...
def apply_frame_transforms(
    ...
    *,
    ...
    if train:
        # Store non-augmented copies of images before augmentation (for hash-based LAM lookup)
        # Use "noaug_" prefix (not "image_") to avoid being picked up by augment()
        def store_noaug(frame: dict):
            for key in list(frame["observation"].keys()):
                if key.startswith("image_"):
                    frame["observation"][f"noaug_{key}"] = frame["observation"][key]
            return frame

        dataset = dataset.frame_map(store_noaug, num_parallel_calls)
