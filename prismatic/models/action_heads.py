...
class LAMCodeHead(nn.Module):
    """MLP-based head that predicts LAM codes (4 tokens × 16 categories) via cross-entropy."""
    def __init__(
        self,
        input_dim=4096,
        hidden_dim=4096,
        ...
        super().__init__()
        self.num_task_tokens = num_task_tokens
        self.num_lam_tokens = num_lam_tokens
        self.num_lam_categories = num_lam_categories
        self.action_dim = action_dim
        self.hidden_dim = hidden_dim
        # Output dimension is 4 * 16 = 64 (4 LAM tokens, each with 16 category logits)
        self.output_dim = num_lam_tokens * num_lam_categories
        self.model = MLPResNet(
            num_blocks=24,
            input_dim=input_dim * ACTION_DIM,
            hidden_dim=hidden_dim,
    ...
    def predict_lam_code(
        ...
        """
        Predict LAM codes from hidden states.

        Args:
            actions_hidden_states: Hidden states from VLM, shape (batch, num_layers, seq_len, hidden_dim)
            proprio: Proprioceptive state (optional)
            proprio_projector: Projector for proprio (optional)
        ...
        # Output shape: (batch, chunk_len, output_dim)
        output = self.model(
            rearranged_actions_hidden_states,
            h_a=actions_hidden_states,
            p=proprio_features,
        ...
        # Take the first timestep output (one LAM code per chunk) and reshape to (batch, 4, 16)
        lam_logits = output[:, 0, :]  # (batch, 64)
        lam_logits = lam_logits.reshape(batch_size, self.num_lam_tokens, self.num_lam_categories)  # (batch, 4, 16)

        return lam_logits


class VillaXLatentHead(nn.Module):
    """
    MLP-based head that predicts continuous villa-x latent vectors via MSE/L1 regression.

    Unlike LAMCodeHead which predicts discrete tokens via cross-entropy, this head
    predicts continuous latent vectors (default 512-dim) that serve as a regularization
    target for the VLM hidden states.

    The villa-x latent captures the action dynamics between two frames and is computed
    by villa-x's encoder (IgorEncoder) with shape [1, num_tokens * latent_dim] = [1, 512].
    """
    def __init__(
        self,
        input_dim=4096,
        hidden_dim=4096,
        latent_dim=512,  # villa-x default: 4 tokens × 128 dim = 512
        num_task_tokens=512,
        action_dim=7,
        use_pro_version=False,
    ):
    ...
    def predict_latent(
        ...
        """
        Predict continuous villa-x latent from hidden states.
        ...
        Returns:
            latent: (batch, latent_dim) - continuous latent vector
        """
        batch_size = actions_hidden_states.shape[0]
        device = actions_hidden_states.device
        ...
        # Output shape: (batch, chunk_len, latent_dim)
        output = self.model(
            rearranged_actions_hidden_states,
            h_a=actions_hidden_states,
            p=proprio_features,
        ...
        # Take the first timestep output (one latent per chunk)
        latent = output[:, 0, :]  # (batch, latent_dim)
